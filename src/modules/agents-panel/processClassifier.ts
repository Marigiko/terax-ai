...
export function classifyProcess(process: ProcessInfo): ClassifiedProcess {
  ...
  if (NODE.test(haystack)) {
    return describe(process, "node", "Node", "JavaScript or TypeScript runtime");
  }
  ...
  if (SHELL.test(haystack)) {
    return describe(process, "shell", "Shell", "interactive shell or multiplexer");
  }
  ...
  return describe(process, "binary", "Binary", "native executable or app process");
}
