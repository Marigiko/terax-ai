...
#[cfg(target_os = "macos")]
use objc2_app_kit::{NSApplication, NSImage, NSWindow, NSWindowToolbarStyle};
#[cfg(target_os = "macos")]
use objc2 Foundation::{NSPoint, NSRect, NSSize, NSString};
#[cfg(target_os = "macos")]
...
/// macOS: switch the main window to a unified-compact toolbar style so the
/// title area shrinks and a ⚙️ settings toolbar item can live in the titlebar.
/// The toolbar item itself doesn't carry logic — clicking it emits a frontend
/// event the main window already listens for to open settings.
#[cfg(target_os = "macos")]
pub fn install_titlebar_toolbar<R: Runtime>(app: &AppHandle<R>) {
    ...
    unsafe {
        let ns_window = native as *mut NSWindow;
        let _: () = objc2::msg_send![ns_window, setToolbarStyle: NSWindowToolbarStyle::Compact];
        // Small toolbar — just the settings button, no title text crowding it.
        let _: () = objc2::msg_send![ns_window, setShowsTitle: false];
    }
}

#[cfg(not(target_os = "macos"))]
pub fn install_titlebar_toolbar<R: Runtime>(_app: &AppHandle<R>) {}
